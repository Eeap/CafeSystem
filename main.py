...
class cafeSystem:
    ...
    def searchMain(self,place):
        ...
        if os.path.exists(path):
            ...
            printData = sortData[sortData['menu'].str.contains('아메리카노|카페|라떼|스무디|주스|쥬스|티|차|콜드|아이스|요구르트|브루|에스프레소|모카|ICE|HOT|얼그레이|프라푸치노|커피|블렌디드|핫|카푸치노|돌체')]
            self.driver.quit()
            return printData
    ...
    def search(self,query):
        ...
        try:
            ...
            for idx in range(2,6):
                self.driver.find_element(By.XPATH, '//*[@id="info.search.page.no' + str(idx) + '"]').send_keys(Keys.ENTER)
                sleep(2)
                page = self.driver.page_source
                soup = BeautifulSoup(page, "html.parser")
                lists = soup.select(self.placeList)
    ...
    def getData(self,lists,filewrite):
        for idx, query in enumerate(lists):
            menuData = self.getMenu(idx,filewrite)
    ...
    def getMenu(self,idx,filewrite):
        ...
        path = os.path.join(os.getcwd(),title+'.csv')
        filename = title + ".csv"
            ...
            for item in menu:
                ...
                if price != []:
                    if self.isInt(price):
                        ...
                        result['title']=title
                        print(price, item[0])
                        filewrite.writerow([item[0],price,title])
        self.driver.close()
